...
export interface GLSLError {
	...
	line: number;
	message: string;
	timestamp?: number; // Added optional timestamp to ensure uniqueness
}
...
function createTaskStore() {
	...
	return {
		...
		/**
		 * Sets shader errors, ensuring new errors are tagged with a unique timestamp
		 * to prevent Svelte/Monaco deduplication issues.
		 */
		setShaderErrors(errors: { vertex?: GLSLError[]; fragment?: GLSLError[] }) {
			store.update(s => {
				// Only map new errors, ensuring they get a unique timestamp
				const newErrors = {
					vertex: errors.vertex 
						? errors.vertex.map(e => ({ ...e, timestamp: Date.now() })) 
						: s.shaderErrors.vertex,
					fragment: errors.fragment 
						? errors.fragment.map(e => ({ ...e, timestamp: Date.now() })) 
						: s.shaderErrors.fragment,
				};

				return {
					...s,
					shaderErrors: newErrors,
					_version: Date.now() // Force state update
				};
			});
		},
